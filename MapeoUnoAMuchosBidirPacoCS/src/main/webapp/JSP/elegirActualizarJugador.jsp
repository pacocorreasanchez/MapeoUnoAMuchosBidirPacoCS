...
<html lang="es">
    ...
    <center>
        <div id="principal">
            ...
            <h2>Elige un jugador para modificar</h2>
            <c:if test = "${listaJugadores == null || listaJugadores.size() < 0}">
                <h1>No hay registros en la tabla</h1>
            </c:if>


            <form method="post" action="Controlador?op=updateJugador">
                <center>
                    <table>
                        <c:forEach var="item" items="${listaJugadores}">
                            <tr>
                                <td><input type="radio" name="registroActJ" value="${item.idJugador}" /></td>
